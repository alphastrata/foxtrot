...
for view in "${VIEWS[@]}"; do
    ...
    cargo run --release -p step_thumbnailer -- -i "$INPUT_FILE" -o "$OUTPUT_FILE" -s 512 --view "$view" --engine foxtrot --decimation-ratio=0.18 
done
...
for view in "${VIEWS[@]}"; do
    ...
    cargo run --release -p step_thumbnailer --features occt -- -i "$INPUT_FILE" -o "$OUTPUT_FILE" -s 512 --view "$view" --engine occt
done
