...
struct Args {
    ...
    threads: usize, // 0 means use number of CPU cores
}
...
// Data structure to communicate between threads
...
impl BatchApp {
    ...
    pub fn render_to_texture(
        ...
        let _model_center = glm::vec3(0.0, 0.0, 0.0); // After scaling, center is at origin
...
// Function to process a single file (triangulation only) - this can run in parallel
fn process_file(file_path: &str, gpu_context: &GPUContext) -> Result<(Mesh, Stats), Box<dyn std::error::Error>> {
    println!("Processing: {}", Path::new(file_path).file_name().unwrap().to_string_lossy());
    ...
    // Process using shared GPU context
    let (mesh, stats) = triangulate::wgpu_triangulate::wgpu_triangulate_with_context(&step_file, gpu_context);
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if step_files.is_empty() {
        println!("No STEP files found in {}", args.input_dir);
        return Ok(());
    }
    ...
    println!(
        "Found {} STEP files, processing with {} threads",
        step_files.len(),
        num_threads
    );

    // Initialize GPU context once for all processing (shared across threads)
    let gpu_context = Arc::new(GPUContext::new()?);
    println!("GPU context initialized successfully");

    // Initialize the batch app with proper wgpu device for rendering
    ...
    // Set up MPSC channels for parallel processing
    let (tx, rx) = mpsc::channel();
    
    // Split files among threads
    let files_per_thread = (step_files.len() + num_threads - 1) / num_threads;
    let mut handles = vec![];

    // Create threads for triangulation
    for i in 0..num_threads {
        let start_idx = i * files_per_thread;
        let end_idx = std::cmp::min(start_idx + files_per_thread, step_files.len());
        
        if start_idx >= step_files.len() {
            break; // No more files to process
        }

        let thread_files: Vec<String> = step_files[start_idx..end_idx].to_vec();
        ...
        let handle = thread::spawn(move || {
            for file_path in thread_files {
                ...
                match process_file(&file_path, &gpu_context) {
                    Ok((mesh, stats)) => {
    ...
    // Close the main sender since we don't use it directly
    drop(tx);

    // Process results and render sequentially (since rendering uses shared GPU resources)
    for processed_file in rx {
        if let Some(error) = processed_file.error {
            eprintln!("Error processing {}: {}", processed_file.file_path, error);
            continue;
        }

        if let (Some(mesh), Some(_stats)) = (processed_file.mesh, processed_file.stats) {
            println!("Rendering {}", processed_file.file_path);
            ...
            if mesh.verts.is_empty() || mesh.triangles.is_empty() {
                ...
                let mut file = std::fs::File::create(&processed_file.output_path)?;
                ...
                println!("    Saved to: {}", processed_file.output_path);
            }
            total_processed += 1;
        }
    }
    ...
    println!(
        ...
        total_duration / total_processed.max(1)
    );
    println!(
