...
impl<'a> EntityCache<'a> {
    ...
    fn build_bspline_surface(&mut self, b: &BSplineSurfaceWithKnots_) -> Result<Surface, Error> {
        ...
        let control_points_list = control_points_2d(self.step_file, &b.control_points_list);
    ...
    fn build_nurbs_surface(&mut self, v: &[Entity<'a>]) -> Result<Surface, Error> {
        ...
        let control_points_list = control_points_2d(self.step_file, &bspline.control_points_list)
            .into_iter()
...
// triangulate5 has been moved to historical_triangulations module
...
/// Triangulates a STEP file into a mesh with statistics.
/// This is the primary public API for triangulation.
#[cfg(feature = "rayon")]
...
/// Triangulates a STEP file into a mesh with statistics.
/// This is the primary public API for triangulation (without rayon).
#[cfg(not(feature = "rayon"))]
pub fn triangulate(s: &StepFile) -> (Mesh, Stats) {
    // Fallback to the original triangulation implementation
    ...
    let (to_mesh_iter, empty) = { (to_mesh.iter(), (Mesh::default(), Stats::default())) };
    ...
    let (mesh, stats) = mesh_fold;

    log::info!("num_shells: {}", stats.num_shells);
    log::info!("num_faces: {}", stats.num_faces);
    log::info!("num_errors: {}", stats.num_errors);
    log::info!("num_panics: {}", stats.num_panics);

    (mesh, stats)
}

// triangulate4 (older implementation) has been moved to historical_triangulations module
...
pub fn open_shell(s: &StepFile, c: OpenShell, mesh: &mut Mesh, stats: &mut Stats) {
...
pub fn closed_shell(s: &StepFile, c: ClosedShell, mesh: &mut Mesh, stats: &mut Stats) {
...
#[cfg(feature = "rayon")]
// triangulate2 has been moved to historical_triangulations module
#[cfg(feature = "rayon")]
// triangulate3 has been moved to historical_triangulations module
...
#[cfg(feature = "rayon")]
/// Optimized triangulation implementation (version 4)
/// This is the current recommended implementation for production use.
pub fn triangulate4(s: &StepFile) -> (Mesh, Stats) {
    use rayon::prelude::*;
    use std::sync::atomic::{AtomicUsize, Ordering};

    // Phase 1: Build lookup cache for the whole StepFile
    let entity_cache = EntityCache::new(s);

    // Phase 2: Build face catalog with minimal allocations
    let brep_colors: AHashMap<_, DVec3> =
        s.0.iter()
            .filter_map(MechanicalDesignGeometricPresentationRepresentation_::try_from_entity)
            .flat_map(|m| m.items.iter())
    ...
    // Phase 3: Extract all face IDs with metadata (no deep copies)
    struct FaceTask<'a> {
    ...
    // Phase 4: Parallel triangulation using pre-built cache
    ...
    let mesh = face_tasks
        ...
        .filter_map(|task| {
            ...
            let mut local_cache = entity_cache.clone();
            match triangulate_single_face_cached(
                s,
                &mut local_cache,
                task.face_id,
                &mut verts,
...
fn curve_cached(
    ...
    Ok(match &s[curve_id] {
        Entity::Circle(c) => {
            let (location, axis, ref_direction) = cache.axis2_placement_3d(c.position.cast());
            Curve::new_circle(
                location,
                axis,
                ref_direction,
        ...
        Entity::SurfaceCurve(v) => curve_cached(s, cache, edge_curve, v.curve_3d, orientation)?,
        Entity::SeamCurve(v) => curve_cached(s, cache, edge_curve, v.curve_3d, orientation)?,
        // The Line type ignores pnt / dir and just uses u and v
        Entity::Line(_) => Curve::new_line(),
        e => {
            warn!("Could not get edge from {:?}", e);
...
#[cfg(feature = "rayon")]
fn edge_curve_cached(
    s: &StepFile,
    cache: &mut EntityCache,
    e: EdgeCurve,
    orientation: bool,
) -> Result<Vec<DVec3>, Error> {
    // Clone the edge_curve reference to avoid borrowing issues
    let edge_curve = *cache.edge_curves.get(&e).expect("Could not get EdgeCurve");
    let curve = curve_cached(s, cache, &edge_curve, edge_curve.edge_geometry, orientation)?;
...
fn edge_loop_cached(
    ...
    for (i, e) in edge_list.iter().enumerate() {
        ...
        let edge = cache
            .oriented_edges
            .get(e)
            .expect("Could not get OrientedEdge");
        let o = edge_curve_cached(s, cache, edge.edge_element.cast(), edge.orientation)?;
        out.extend(o.into_iter());
    }
    Ok(out)
}
...
fn face_bound_cached(
    ...
    match &s[bound] {
        Entity::EdgeLoop(e) => {
            let mut d = edge_loop_cached(s, cache, &e.edge_list)?;
            if !orientation {
                d.reverse()
            }
            Ok(d)
        ...
        Entity::VertexLoop(v) => {
            // This is an "edge loop" with a single vertex, which is
            // used for cones and not really anything else.
            Ok(vec![cache.vertex_point(v.loop_vertex)])
        }
        e => panic!("{:?} is not an EdgeLoop", e),
    }
}
...
#[cfg(feature = "rayon")]
// Core triangulation - optimized hot path with EntityCache
fn triangulate_single_face_cached<'a>(
    s: &'a StepFile<'a>,
    ...
    for b in &face.bounds {
        let bound_contours = face_bound_cached(s, cache, *b)?;
    ...
    let t = loop {
        match cdt::Triangulation::new_with_edges(&pts, &edges) {
            Ok(mut t) => match t.run() {
                ...
                Err(cdt::Error::PointOnFixedEdge(_)) => return Err(Error::PointOnFixedEdge),
                Err(cdt::Error::NoMorePoints) => return Err(Error::NoMorePoints),
                Err(cdt::Error::CrossingFixedEdge) => return Err(Error::CrossingFixedEdge),
                Err(cdt::Error::EmptyInput) => return Err(Error::EmptyInput),
                Err(cdt::Error::InvalidInput) => return Err(Error::InvalidInput),
                Err(cdt::Error::InvalidEdge) => return Err(Error::InvalidEdge),
                Err(cdt::Error::OpenContour) => return Err(Error::OpenContour),
                Err(cdt::Error::TooFewPoints) => return Err(Error::TooFewPoints),
                Err(cdt::Error::CannotInitialize) => return Err(Error::CannotInitialize),
                Err(cdt::Error::WedgeEscape) => return Err(Error::WedgeEscape),
                Err(cdt::Error::HullMismatch) => return Err(Error::HullMismatch),
            },
            Err(cdt::Error::EmptyInput) => return Err(Error::EmptyInput),
            Err(cdt::Error::InvalidInput) => return Err(Error::InvalidInput),
            Err(cdt::Error::InvalidEdge) => return Err(Error::InvalidEdge),
