...
pub async fn gpu_raise_and_transform(
    ...
    match receiver.recv_timeout(std::time::Duration::from_secs(5)) {
        ...
        Ok(Err(e)) => {
            trace!("gpu_raise_and_transform: Buffer mapping failed: {:?}", e);
            Err(format!("Buffer mapping failed: {:?}", e).into())
        },
        Err(_) => {
            trace!("gpu_raise_and_transform: Buffer mapping timed out");
            Err("Buffer mapping timed out".into())
        },
    }
}
...
mod tests {
    ...
    fn test_gpu_lowering_and_raising() {
        trace!("test_gpu_lowering_and_raising: Starting test");
        ...
        let (device, queue) = match create_wgpu_device() {
            Ok(dq) => {
                trace!("test_gpu_lowering_and_raising: Created wgpu device and queue");
                dq
            },
            Err(_) => {
                trace!("test_gpu_lowering_and_raising: Skipping GPU test: could not create wgpu device.");
                return;
            }
        };
        ...
        trace!("test_gpu_lowering_and_raising: Created surface");
        ...
        trace!("test_gpu_lowering_and_raising: Created {} test vertices", vertices.len());

        // 3. Lower vertices to UVs
        trace!("test_gpu_lowering_and_raising: Starting GPU lowering");
        let uvs = pollster::block_on(gpu_lower_vertices(&device, &queue, &vertices, &surface))
            .expect("GPU lowering failed");
        trace!("test_gpu_lowering_and_raising: GPU lowering completed, got {} UVs", uvs.len());

        // For a simple XY plane, we should get 2 UV coordinates back
        assert_eq!(uvs.len(), 2);
        trace!("test_gpu_lowering_and_raising: Expected UV[0] = (1.0, 2.0), got ({}, {})", uvs[0].0, uvs[0].1);
        trace!("test_gpu_lowering_and_raising: Expected UV[1] = (3.0, 4.0), got ({}, {})", uvs[1].0, uvs[1].1);
        ...
        trace!("test_gpu_lowering_and_raising: UV validation passed (finite values check)");
        ...
        trace!("test_gpu_lowering_and_raising: Test completed successfully");
    }
}